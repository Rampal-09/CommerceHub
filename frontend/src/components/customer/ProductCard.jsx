...
export const ProductCard = ({ product, index = 0 }) => {
  ...
  return (
    <div
      className="group bg-white rounded-3xl border border-slate-200/90 overflow-hidden shadow-2xs hover:shadow-xl hover:-translate-y-1 hover:border-indigo-200/60 transition-all duration-300 flex flex-col h-full relative animate-rise-in"
      style={{ '--delay': `${Math.min(index, 11) * 60}ms` }}
    >
      ...
      <div className="relative w-full aspect-square bg-slate-50 overflow-hidden shrink-0">
        ...
        {/* Discount Badge */}
        {hasDiscount && (
          <div className="absolute top-3 right-3 z-10 px-2.5 py-1 bg-gradient-to-r from-red-500 to-rose-600 text-white text-[11px] font-black rounded-full shadow-sm">
            -{discountPercent}%
          </div>
        )}
      ...
      <div className="p-5 flex flex-col flex-1 justify-between space-y-4">
        ...
        <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
          ...
          <button
            ...
            className="px-3.5 py-2 bg-brand-gradient text-white rounded-xl text-xs font-bold transition-all shadow-brand-glow shadow-brand-glow-hover hover:-translate-y-0.5 active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-1.5 cursor-pointer shrink-0"
